...
function getEconomyRange(seed, rarity, permanent)
    ...
    rarity = rarity or getRarity()
    if permanent == nil then permanent = getPermanent() end
...
function getName(seed, rarity)
    ...
    local version = rarity.value + 1
    local patch = makeSerialNumber(seed, 2, nil, nil, "0123456789")
    local vars = {version = version, patch = patch}

    local prefix = ""
    if rarity.value == 0 then
        return "Basic Trading Subsystem v${version}.${patch}"%_t % vars
    elseif rarity.value == 1 then
        return "Improved Trading Subsystem v${version}.${patch}"%_t % vars
    elseif rarity.value == 2 then
        return "Advanced Trading Subsystem v${version}.${patch}"%_t % vars
    elseif rarity.value == 3 then
        return "High-Tech Trading Subsystem v${version}.${patch}"%_t % vars
    elseif rarity.value == 4 then
        return "Salesman's Trading Subsystem v${version}.${patch}"%_t % vars
    elseif rarity.value == 5 then
        return "Ultra-Tech Trading Subsystem v${version}.${patch}"%_t % vars
    end

    return "Broken Trading Subsystem vX.X"%_t
end

function getBasicName()
    return "Trading Subsystem /* generic name for 'Basic Trading Subsystem v${version}.${patch}' */"%_t
end
